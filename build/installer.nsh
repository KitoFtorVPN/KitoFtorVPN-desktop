...
;
; nsExec (not ExecWait) is used here on purpose: ExecWait runs sc.exe /
; taskkill.exe with their normal console window, which flashes on screen
; for a fraction of a second during both install and update — confusing
; for users who didn't ask to see a command prompt. nsExec runs the exact
; same commands, waits for them the same way, but redirects their console
; so nothing is shown. The stop logic and its ordering are unchanged.
; ───────────────────────────────────────────────────────────
!macro customInit
